		...
		<?php
			...
			<?php
					...
					if (mysqli_num_rows($row)>0)
					{
						...
						<section class="evsec">
							<a href="admineventinfo.php?id=<?php echo $id; ?>" class="pa" >
								<table width="100%" height=100% bgcolor="white">
									...
									<tr>
										<td align="center"> <a href="admineditevent.php?id=<?php echo $id; ?>" class='btn1'> Edit </a> </td>
										<td align="center"> <a href="admindeleteevent.php?id=<?php echo $id; ?>" class='btn1'> Delete </a> </td>
									</tr>
			...
			<table align="right">
				...
				<tr>
					<form action="adminsearch.php">
						...
						<td>
							<input type="text" name="search" placeholder="Enter event name" class="search"> &nbsp &nbsp; &nbsp; &nbsp;
						</td>
   	...
   	<?php
   		if(isset($_POST['filter']))
   		{
   			...
   			if($date == 'All')
   			{
   				...
   				echo "<script> location.href='adminallevent.php?muk=".$muk."' </script>";
   			}
   			...
   			elseif($date == 'Weekend')
   			{
   				...
   				elseif($cat!='All' && $price == 'All')
   				{
   					$sql = "select * from events WHERE Start_Date>'".date('Y-m-d')."' and dayofweek(Start_Date) = 7 or dayofweek(Start_Date) = 1 and Event_Category='".$cat."' and not E_Delete='yes' order by Priority;";
   				}
   				elseif ($cat=='All' && $price!='All') 
   				{
   					if($price == 'Free')
   					{
   						$sql = "select * from events WHERE Start_Date>'".date('Y-m-d')."' and dayofweek(Start_Date) = 7 or dayofweek(Start_Date) = 1 and Price=0 and not E_Delete='yes' order by Priority;";
   					}
   					elseif($price == 'Paid')
   					{
   						$sql = "select * from events WHERE Start_Date>'".date('Y-m-d')."' and dayofweek(Start_Date) = 7 or dayofweek(Start_Date) = 1 and not E_Delete='yes' and Price>0 order by Priority;";
   					}
   				}
   				elseif($cat!='All' && $price!='All')
   				{
   					if($price == 'Free')
   					{
   						$sql = "select * from events WHERE Start_Date>'".date('Y-m-d')."' and dayofweek(Start_Date) = 7 or dayofweek(Start_Date) = 1 and Event_Category='".$cat."' and Price=0 and not E_Delete='yes' order by Priority;";
   					}
   					elseif($price == 'Paid')
   					{
   						$sql = "select * from events WHERE Start_Date>'".date('Y-m-d')."' and dayofweek(Start_Date) = 7 or dayofweek(Start_Date) = 1 and Event_Category='".$cat."' and Price>0 and not E_Delete='yes' order by Priority;";
