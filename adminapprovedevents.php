<html>
	...
	<body>
		...
		<div class="eva" id="s1">
			...
			<?php
        			$sql = "select * from publisher where Start_Date>'".date('Y-m-d')."' and not c_delete='yes' and verify='yes';";
						...
						<section class="evsec">
							<a href="userevent.php?id=<?php echo $id; ?>" class="pa" >
								<table width="100%" height=100% bgcolor="white">
									...
									<tr>
										<td colspan="2" align="right" style="padding-right: 10px;"> <?php echo $payment; ?> </td>
									</tr>

									<tr>
										<td colspan="2" align="left" style="padding-left: 10px;" id='add<?php echo $r['Event_ID']; ?>'> <a href="adduserevent.php?id=<?php echo $id ?>" class='btn'> Add </a> <br> </td>
									</tr>
					...
					if($r['added']=='yes')
					{
						echo "<script>document.getElementById('add".$r['Event_ID']."').style.display='none'; </script>";
					}
				}
			}
	    	else
   	...
   	<?php
   		if(isset($_POST['filter']))
   		{
   			...
   			if($cat=='All')
   			{
   				...
   				elseif ($price == 'Free') 
   				{
   					$sql = "select * from publisher where Start_Date>'".date('Y-m-d')."' and Price=0 and not c_delete='yes' and verify='yes';";
   				}
   				else
   				{
   					$sql = "select * from publisher where Start_Date>'".date('Y-m-d')."' and Price>0 and not c_delete='yes' and verify='yes';";
   				...
   				echo "<script> location.href='adminapprovedevents.php?muk=".$muk."' </script>";
   			}
   			elseif($cat != 'All')
   			{
   				if($price == 'All')
   				{
   					$sql = "select * from publisher where Start_Date>'".date('Y-m-d')."' and Event_Category='".$cat."' and not c_delete='yes' and verify='yes';";
   				}
   				elseif ($price == 'Free') 
   				{
   					$sql = "select * from publisher where Start_Date>'".date('Y-m-d')."' and Price=0 and Event_Category='".$cat."' and not c_delete='yes' and verify='yes';";
   				}
   				else
   				{
   					$sql = "select * from publsiher where Start_Date>'".date('Y-m-d')."' and Price>0 and Event_Category='".$cat."' and not c_delete='yes' and verify='yes';";
   				}
   				$muk = base64_encode($sql);
